...
void UnistdOpen(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
    ReturnValue->Val->Integer = open(Param[0]->Val->Pointer,Param[1]->Val->Integer);
}

void UnistdOpen2(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
    ReturnValue->Val->Integer = open(Param[0]->Val->Pointer,Param[1]->Val->Integer,Param[2]->Val->Integer);
}

/* handy structure definitions */
const char UnistdDefs[] = "typedef int uid_t; \
typedef int gid_t; \
typedef int pid_t; \
typedef int off_t; \
typedef int size_t; \
...
/*    { UnistdSwab,          "void swab(void *, void *, ssize_t);" }, */
    ...
    { UnistdOpen,	   "int open(char *, int);"},
    { UnistdOpen2,	   "int open2(char*, int,int);"},
    { NULL,                 NULL }
};
