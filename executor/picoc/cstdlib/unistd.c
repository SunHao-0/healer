...
void UnistdPread(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs) {
    ReturnValue->Val->Integer = pread(Param[0]->Val->Integer, Param[1]->Val->Pointer, Param[2]->Val->Integer,
                                      Param[3]->Val->Integer);
}

void UnistdPwrite(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs) {
    ReturnValue->Val->Integer = pwrite(Param[0]->Val->Integer, Param[1]->Val->Pointer, Param[2]->Val->Integer,
                                       Param[3]->Val->Integer);
}
...
void UnistdGetgroups(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs) {
    ReturnValue->Val->Integer = getgroups(Param[0]->Val->Integer, (gid_t *) Param[1]->Val->Pointer);
}

void UnistdPipe(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs) {
    int pipefds[2];
    int ret = pipe(pipefds);
    ReturnValue->Val->Integer = ret;// pipe(Param[0]->Val->Pointer);
}
...
/*    { UnistdExecvp,        "int execvp(char *, char *[]);" }, */
                ...
                {UnistdGetgroups,     "int getgroups(int, gid_t list []);"},
...
/*    { UnistdGetopt,        "int getopt(int, char * [], char *);" }, */
                ...
                {UnistdPipe,          "int pipe(int pipefd[2]);"},
